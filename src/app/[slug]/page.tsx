...
import type { Breeding, Dog } from "@/lib/supabase/types";
...
function toWhatsAppHref(rawPhone: string) {
  const digits = rawPhone.replace(/\D/g, "");
  return `https://wa.me/${digits}`;
}

function toSocialHref(value: string, baseUrl: string) {
...
export default async function KennelPage({ params }: PageProps<"/[slug]">) {
  ...
  // Orden fijo de secciones: Studs, Females, Productions (incluye
  // cachorros), Breedings, Available.
  ...
  const contactLinks = [
    kennel.whatsapp && { label: "WhatsApp", href: toWhatsAppHref(kennel.whatsapp) },
    kennel.phone && { label: "Call", href: `tel:${kennel.phone}` },
    kennel.email && { label: "Email", href: `mailto:${kennel.email}` },
    kennel.instagram && {
      label: "Instagram",
      href: toSocialHref(kennel.instagram, "https://instagram.com"),
    },
    kennel.facebook && {
      label: "Facebook",
      href: toSocialHref(kennel.facebook, "https://facebook.com"),
    },
              ...
              <div className="flex flex-wrap items-center justify-center gap-2.5">
                {contactLinks.map((link) => (
                  <a
                    ...
                    className="rounded-full bg-[var(--color-accent)] px-6 py-3 font-body text-sm font-extrabold uppercase tracking-wide text-ink-text transition-colors hover:bg-[var(--color-accent-dim)]"
                  >
                    {link.label}
                  </a>
                ))}
